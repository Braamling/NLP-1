...
class Text_Generator():
    ...
    def generate_from(self, starting_text):
        ...
        with tf.Session() as session:  
            ...
            saver.restore(session, self.gen_config.session_name)

            while starting_text:
                print ' '.join(self.generate_sentence(
                        session, self.gen_model, self.gen_config, starting_text=starting_text, temp=1.0))
                starting_text = raw_input('> ')


    def generate_text(self, session, model, config, starting_text='<eos>',
